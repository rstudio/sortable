...
You can create a drag-and-drop input object in Shiny, using the `sortable_list()` function.


```{r echo=FALSE, cache=FALSE}
knitr::read_chunk(
  system.file("shiny-examples/sortable_list_app.R", package = "sortable")
)
```

```{r sortable-list-app, eval=FALSE}
```
...
Witha a bucketable list you can have more than one sortable lists in a single object.  This can be useful for bucketing task, e.g. asking your students to classify objects into multiple categories.

```{r echo=FALSE, cache=FALSE}
knitr::read_chunk(
  system.file("shiny-examples/bucketable_list_app.R", package = "sortable")
)
```

```{r bucketable-list-app, eval=FALSE}
```
