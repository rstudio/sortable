...
<!-- badges: start -->
[![Travis build status](https://travis-ci.org/timelyportfolio/sortableR.svg?branch=master)](https://travis-ci.org/timelyportfolio/sortableR)
...
The goal of `sortable` is to expose the functionality of `sortable.js` as an [htmlwidget](https://htmlwidgets.org) in R, so you can use this in Shiny apps and widgets, as well as R Markdown.

[Sortable.js](https://rubaxa.github.io/Sortable) is so ridiculously amazing that of course we need it in **R** as an [htmlwidget](https://htmlwidgets.org).  `sortable` attempts to harness some of the power of `Sortable.js`.
...
Or, check out these reorderable tabs by `runGist("2dbe45f77b65e28acab9")`.  All we had to do was add an `id` and add one line of code to the [Tabset example](https://github.com/rstudio/shiny-examples/tree/master/006-tabsets) from [RStudio](https://rstudio.com).
